...
from training.val_dataset import BirdDataset, BirdSEDDataset
...
def create_data_datasets(args):
    ...
    ## set 1 csv
    train_dataset = BirdSEDDataset(mode="train", folds_csv=args.folds_csv, dataset_dir=args.data_dir, fold=args.fold,
                                multiplier=conf.get("multiplier", 1), duration=train_period, transforms=train_transforms,
                                n_classes=conf['encoder_params']['classes'])
    val_dataset = BirdSEDDataset(mode="val", folds_csv=args.folds_csv, dataset_dir=args.data_dir, fold=args.fold, duration=infer_period,
                              n_classes=conf['encoder_params']['classes'])
