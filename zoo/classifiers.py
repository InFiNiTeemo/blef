...
class SED(nn.Module):
    def __init__(self, encoder: str,
        ...
        self.amplitude_to_db = ta.transforms.AmplitudeToDB(top_db=mel_config['top_db'])  # 不改变维度
...
class TimmClassifier(nn.Module):
    def __init__(self, encoder: str,
        ...
        #  b, t ->  b, mel, time  这里time也可以理解为frame
        #  除最后一维大小不变 e.g.  b, c, t -> b, c, mel, time
    ...
    def forward(self, x, is_test=False):
        ...
        if self.training:
            ...
            x = x.reshape(b // self.factor, self.factor * t, c, f) # 在之前首先乘了FACTOR使得前后之间没有依赖关系
